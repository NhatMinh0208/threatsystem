...
function DarkRP.hooks:canArrest(arrester, arrestee)
    ...
    if GAMEMODE.Config.needwantedorthreat and not arrestee:IsNPC() and not arrestee:getDarkRPVar("wanted") and not arrestee:isThreat() then
        return false, DarkRP.getPhrase("must_be_wanted_or_threat")
    end
    if not GAMEMODE.Config.needwantedorthreat then
        ...
        if GAMEMODE.Config.needthreatforarrest and not arrestee:IsNPC() and not arrestee:isThreat() then
            return false, DarkRP.getPhrase("must_be_threat_for_arrest")
        end
    end
